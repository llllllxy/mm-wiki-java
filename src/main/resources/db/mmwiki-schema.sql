...
CREATE TABLE `mw_document` (
  ...
  `path` varchar(3000) NOT NULL DEFAULT '0' COMMENT '存储根文档到父文档的 document_id 值, 格式 0,1,2,...',
...
CREATE TABLE `mw_log` (
  ...
  `level` tinyint(4) NOT NULL DEFAULT '6' COMMENT '日志级别',
...
CREATE TABLE `mw_login_auth` (
  `login_auth_id` int(10) NOT NULL AUTO_INCREMENT COMMENT '认证表主键ID',
